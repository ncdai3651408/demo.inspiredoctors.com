...
class BookingCanceledEventHandler
{
    ...
    public static function handle($commandResult, $container)
    {
        ...
        try {
            $googleCalendarService->handleEvent(AppointmentFactory::create($appointment), self::BOOKING_CANCELED);
        } catch (\Exception $e) {
        }

        $emailNotificationService->sendCancelBookingNotification($appointment, $booking);

        if ($settingsService->getSetting('notifications', 'smsSignedIn') === true) {
            $smsNotificationService->sendCancelBookingNotification($appointment, $booking);
        }
